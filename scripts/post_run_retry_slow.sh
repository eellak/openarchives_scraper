...
# Wait for the most recent collector (normal or retry) to finish
...
# Launch slower retry: concurrency 1, wait 1s
RTS="$(date +%Y%m%d_%H%M%S)"
CONSOLE_LOG="$LOG_DIR/console_collect_all_${RTS}_retry2.log"
PID_OUT="$LOG_DIR/collect_all_${RTS}_retry2.pid"
...
echo "Retry (slow) launched. Console: $CONSOLE_LOG PID: $(cat "$PID_OUT")"
